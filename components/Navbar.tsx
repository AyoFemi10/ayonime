...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 nav-blur bg-ayo-bg/80 border-b border-ayo-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-sm">
          <div className="relative w-full">
            ...
            />
            <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-ayo-muted hover:text-ayo-accent transition-colors" aria-label="Search">
        ...
        <div className="flex md:hidden items-center gap-2">
          {/* Mobile search icon */}
          <button
            className="text-ayo-muted hover:text-white p-1.5"
      ...
      {searchOpen && (
        <div className="md:hidden bg-ayo-surface border-t border-ayo-border px-4 py-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            ...
            />
            <button type="submit" className="px-4 py-2 bg-ayo-gradient rounded-lg text-white text-sm font-bold">Go</button>
          </form>
        </div>
      )}
